...
def get_fastspar_commands(fname, outdir, fastspar_args):
...
def run_fastspar_all(fnames, outdir, fastspar_args):
    for f in fnames:
        logger.log("Running FastSpar for file: {f}", bcolors.OKGREEN)
        cmds = get_fastspar_commands(f, outdir, fastspar_args)
        for c in cmds:
            run_command(c)
...
def main():
    ...
    original_name = os.path.basename(abnfile).replace('.tsv', '')

    fnames = write_split_data(splitdf, original_name, outdir)

    run_fastspar_all(fnames, outdir, fastspar_args)
